...
func (r *Handle) tokenKey(token string) string {
...
func (r *Handle) DestroyToken(token string) error {
	...
	println(expired)
	return redisplus.Redis.String.SetEX(r.tokenKey(token), "1", int64(expired)).Error()
}
...
func (r *Handle) ParseToken(token string) (*Userdata, error) {
	...
	exists, err := redisplus.Redis.Key.Exists(r.tokenKey(token)).Bool()
	if err != nil {
		return nil, err
	}
	if exists {
...
func NewHandle(conf *Config) *Handle {
	if conf == nil {
		handleLock.RLock()
		conf = defaultConfig
		conf.key = []byte("https://gitee.com/xinglou123/xl_kit")
		handleLock.RUnlock()
	}
	return &Handle{
		Config: conf,
